...
async def get_cpp_rt_flags():
    ...
    with tempfile.TemporaryDirectory(
        dir=get_tmp_dir(), ignore_cleanup_errors=True
    ) as tmp_dir:
        with tempfile.NamedTemporaryFile(
            mode="w", dir=tmp_dir, suffix=".cpp", delete=False
        ) as f:
        ...
        for flag in optional_flags:
            compile_res = await run_command_bare(
                f"g++ {f.name} -o test {flag}", cwd=tmp_dir
            )
            if (
                compile_res.status == CommandRunStatus.Finished
                and compile_res.return_code == 0
            ):
...
async def run_cpp(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(
        ...
        return await run_commands(
            f'g++ -std=c++17 {f.name} -o test {" ".join(flags)}',
            "./test",
            tmp_dir,
            {},
            args,
...
async def run_csharp(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(
        ...
        await run_command_bare(
            create_project_command, timeout=args.compile_timeout, cwd=tmp_dir
        )
        ...
        return await run_commands(
            None, f"dotnet run --project {tmp_dir}", tmp_dir, {}, args
        )
...
async def run_go_test(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(
        ...
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(
            mode="w", dir=tmp_dir, suffix="_test.go", delete=False
        ) as f:
            f.write(args.code)
...
async def run_go(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(
        ...
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(
            mode="w", dir=tmp_dir, suffix=".go", delete=False
        ) as f:
            f.write(args.code)

        return await run_commands(
            f"go build -o out {f.name}", "./out", tmp_dir, {}, args
        )
...
async def run_java(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(
        ...
        restore_files(tmp_dir, args.files)
        jars = [".", "javatuples-1.2.jar"] + [
            filename for filename in args.files.keys() if filename.endswith(".jar")
        ]
        ...
        return await run_commands(
            f"javac {cpargs} Main.java", f"java {cpargs} -ea Main", tmp_dir, {}, args
        )
...
async def run_junit(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(
        ...
        return await run_commands(
            f"javac -cp {cpargs} *.java",
            f"java -jar ./{junit_jar} --class-path {cpargs} --scan-class-path",
            tmp_dir,
            {},
            args,
        )
...
async def run_nodejs(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(
        ...
        restore_files(tmp_dir, args.files)
        os.symlink(
            os.path.join(deps_dir, "node_modules"),
            os.path.join(tmp_dir, "node_modules"),
        )
        with tempfile.NamedTemporaryFile(
            mode="w", dir=tmp_dir, suffix=".js", delete=False
        ) as f:
            f.write(args.code)
...
async def run_typescript(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", dir=tmp_dir, suffix=".ts", delete=False
        ) as f:
            f.write(args.code)
...
async def run_jest(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", dir=tmp_dir, suffix=".test.ts", delete=False
        ) as f:
            f.write(args.code)
...
async def run_php(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(
        ...
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(
            mode="w", dir=tmp_dir, suffix=".php", delete=False
        ) as f:
...
async def run_rust(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(
        ...
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(
            mode="w", dir=tmp_dir, suffix=".rs", delete=False
        ) as f:
            f.write(args.code)

        return await run_commands(
            f"rustc {f.name} -o test", "./test", tmp_dir, {}, args
        )
